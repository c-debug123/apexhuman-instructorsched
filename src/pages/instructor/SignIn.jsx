...
export default function SignIn() {
  ...
  const existingNames = useMemo(() => {
    try {
      const claims = JSON.parse(localStorage.getItem('apex_claims')) || []
      return [...new Set(claims.map(cl => cl.instructorName).filter(Boolean))].sort()
    } catch { return [] }
  }, [])
  ...
  function handleSubmit(e) {
    ...
    localStorage.setItem('apex_instructor_name', trimmed)
    navigate('/schedule/slots')
  }

  function selectName(n) {
    localStorage.setItem('apex_instructor_name', n)
    navigate('/schedule/slots')
  }
      ...
      <div className="z1" style={{ width: '100%', maxWidth: 400 }}>
              ...
              {existingNames.map(n => (
                ...
                >
                  <div style={{
                    ...
                    background: 'var(--teal-dim)', border: '1px solid var(--teal-border)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontFamily: 'Space Grotesk', fontWeight: 700, fontSize: 13, color: 'var(--teal)',
                  }}>
                    {n[0].toUpperCase()}
                  </div>
                  <span style={{ fontFamily: 'Space Grotesk', fontWeight: 600, fontSize: 14, color: 'var(--text-1)', flex: 1 }}>
                    {n}
                  </span>
        ...
        <div className="card" style={{ padding: 28 }}>
          <h1 style={{ fontFamily: 'Space Grotesk', fontWeight: 700, fontSize: 20, color: 'var(--text-1)', marginBottom: 6 }}>
            {existingNames.length > 0 ? 'Or enter a new name' : 'Welcome'}
          </h1>
          <p style={{ fontSize: 14, color: 'var(--text-3)', marginBottom: 20, lineHeight: 1.5 }}>
            {existingNames.length > 0
              ? 'First time? Enter your name below.'
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: 20 }}>
              ...
              <input
                ...
                autoFocus={existingNames.length === 0}
                value={name}
